...
func make_sec_error(_ err: OSStatus, _ message: String) -> NSError
{
    ...
    let error = NSError(domain: kCFErrorDomainOSStatus as String, code: Int(err), userInfo: userInfo as? [String : Any])
...
func make_error(_ err: CFError) -> NSError
{
    ...
    let error = NSError(domain: domain as String, code: code, userInfo: userInfo as? [String : Any] )
...
struct Keychain
{
    ...
    func DeleteItem(item: SecKeychainItem ) throws
    {
        let err = SecKeychainItemDelete(item)
        
        if err != errSecSuccess {
            throw make_sec_error(err, "Cannot delete certificate '\(item)'")
        }
    }
